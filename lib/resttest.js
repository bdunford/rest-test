...
function RestTest() {
            ...
            },
            {
                ...
                parser: function(req,res,next) {
                    bodyParser.text({type: content})(req,res,function(){
                        req.body = xml.parse(req.body,true);
                        next();
                    });
                },
                type: 'application/xml',
                formatter: function(data){
                    var root = Array.isArray(data) && args.noun || xml.child(args.noun);
                    return xml.stringify(data, root);
                },
    ...
    var _get = function(args) {
        var byId = args.id && {id: args.id};
        var results = args.store.find(byId);
        return byId && results && results.length == 1 && results[0] || results || [];
    };
